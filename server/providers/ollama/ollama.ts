...
class OllamaProvider extends Provider {
  ...
  const fetchImpl = ollamaFetch.current as unknown as typeof globalThis.fetch | undefined;
  const ollama = new Ollama({ host, fetch: fetchImpl });
    return ollama;
  }
  ...
  const fetchImpl = ollamaFetch.current as unknown as typeof globalThis.fetch | undefined;
  const ollama = new Ollama({ host: process.env.MAKYO_OLLAMA_HOST, fetch: fetchImpl });
